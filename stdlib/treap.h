...
type TreapSetNode[T]      = treap_set_leaf,
                            treap_set_node(
                              elem:      T,
                              priority:  Nat,
                              left:      TreapSetNode[T],
...
type TreapMapNode[KT, VT] = treap_map_leaf,
                            treap_map_node(
                              key:       KT,
                              value:     VT,
                              priority:  Nat,
...
empty_treap_set = treap_set(size: 0, root: :treap_set_leaf);
...
TreapSet[T] insert(TreapSet[T] set, T elem, (T, T -> Bool) lower_than)
{
  ...
  return if new_root == set.root then set else treap_set(set.size + 1, new_root) end;
}
...
empty_treap_map = treap_map(size: 0, root: :treap_map_leaf);

////////////////////////////////////////////////////////////////////////////////

TreapSetNode[T] treap_set_node(T k, Nat p, TreapSetNode[T] l, TreapSetNode[T] r) =
...
TreapSetNode[T] insert(TreapSetNode[T] root, T elem, (T, T -> Bool) lower_than)
{
...
Bool contains(TreapSetNode[T] treap, T elem, (T, T -> Bool) lower_than)
{
  ...
  branch = if lower_than(elem, treap.elem) then treap.left else treap.right end;
...
Bool has_key(TreapMap[KT, VT] map, KT key, (KT, KT -> Bool) lower_than) =
  try_lookup(map.root, key, lower_than) /= nil;
...
TreapMap[KT, VT] insert(TreapMap[KT, VT] map, KT key, VT value, (KT, KT -> Bool) lower_than)
{
  ...
  return if new_root == map.root then map else treap_map(map.size + 1, new_root) end;
}
...
TreapMapNode[KT, VT] treap_map_node(KT k, VT v, Nat p, TreapMapNode[KT, VT] l, TreapMapNode[KT, VT] r) =
  treap_map_node(key: k, value: v, priority: p, left: l, right: r);

////////////////////////////////////////////////////////////////////////////////

// Nat rand_treap_priority = _rand_nat_(1000000000);
...
TreapMapNode[KT, VT] insert(TreapMapNode[KT, VT] root, KT key, VT value, (KT, KT -> Bool) lower_than)
{
...
{
  return nil if treap == :treap_map_leaf;
  return just(treap.value) if treap.key == key;
  branch = if lower_than(key, treap.key) then treap.left else treap.right end;
