...
(TypeName => AnonType) normalize_and_anonymize_types((TypeSymbol => RawType) inst_types)
{
...
(TypeSymbol => AnonType) anonymize_types((TypeSymbol => RawType) types)
{
  anon_types  := ();
  types_to_do := types;
  ...
  while (types_to_do /= ())
    ...
    assert types_to_do :: (TypeSymbol => RawType);
...
TypeSymbol** type_clusters_to_expand((TypeSymbol => RawType) types, TypeSymbol* already_norm)
{
...
(TypeSymbol => AnonType) expand(TypeSymbol* type_symbs, (TypeSymbol => RawType) types, (TypeSymbol => AnonType) anon_types)
{
  ...
  AnonType expand_type(TypeSymbol type_name, (TypeSymbol => RawType) types, (TypeSymbol => AnonType) anon_types)
  {
    ...
    ;
    type := self_rec_type(type) if has_self_refs;
    return type;
  }

  (TypeSymbol => AnonType) expand_mutually_rec_types((TypeSymbol => RawType) types, (TypeSymbol => AnonType) anon_types)
  {
...
AnonType replace_type_refs(RawType type, (TypeSymbol => RawType) in_progress_type_map, (TypeSymbol => AnonType) subst_map, (TypeSymbol => SelfPretype) rec_subst_map) =
  replace type_ref(ts) in type with replace_type_ref(ts, in_progress_type_map, subst_map, rec_subst_map) end;


AnonType replace_type_ref(TypeSymbol target_type_symbol, (TypeSymbol => RawType) in_progress_type_map, (TypeSymbol => AnonType) subst_map, (TypeSymbol => SelfPretype) rec_subst_map)
{
